...
def build_export_package(config: Config, repo: Repo, model_row: dict[str, Any], onnx_path: Path, *,
    ...
    package_dir = config.exports_dir / f"{slugify(str(model_row['name']))}-{slugify(str(model_row['version']))}"
    if package_dir.exists():
        shutil.rmtree(package_dir)
    package_dir.mkdir(parents=True, exist_ok=True)
    target_model = package_dir / "model.onnx"
    try:
        target_model.hardlink_to(onnx_path)
    except Exception:  # noqa: BLE001 - 跨分区退化为复制

...
### M5 主动学习与扩展 ✅（已完成，验收见 README「测试」）

**实现与验收摘要**
- 代码：`src/rdinspect/active/{scoring,service}.py` + `src/rdinspect/report.py` + CLI `active`/`report` + API `/api/active/*`、`/api/reports/*`；`db/migrations/0002-m5-active-learning.sql`（`runs.kind` 增加 `active`、新增 `active_queue` 表）。
- 验收（`scripts/e2e_m5.py`，**6/6 通过**，含 4 次真实训练，整机 CPU 137 秒）：
  - **新增类别不改代码**：`classes add water_puddle` → 104 个新类标注 → 冻结 → 训练 → 导出，包内 labels 6 类且 ONNX 一致性通过；
  - **主动学习队列**：76 个候选 → 选中 40，写回 `tasks.priority` 与 `active_queue` 明细，`GET /api/tasks?strategy=active` 只看队列；
  - **单位标注量对比**：同预算 40 张新标注，随机选样基线 mAP50 **0.166** vs 主动学习 **0.209**（Δ **+0.043**，每 100 张 +0.055），记录写 `data/reports/active-comparison.json`；
  - **连续两次门禁失败 → 告警**：劣化候选两次被拒（整体 −0.209 + 类别塌陷），`active alerts` 给"先回看标注规范"的处置建议（退出码 5）；
  - **报表**：时间趋势 / 批次对比 / 类别覆盖 Markdown、GeoJSON 255 个点、GIS CSV 表头符合契约。
- 两处门禁加固（本轮发现并修）：① 基线为 0 的类"在候选里缺失"不再算回退；② 基线与候选评估集不一致时显式标注 `comparable=false`（不同冻结集比 mAP 无效）。
...
6. **验收**：新增第五类不改代码即可跑通全链路；主动学习队列使单位标注量的 mAP 增量可测（对比实验记录）。
   > 实测口径：新增类用 `water_puddle`（第 6 类）；增量用同预算 A/B（主动 vs 随机）在**同一冻结测试集**上的 mAP50 差，
   > 并如实记录结论（本机为 +0.043，属合成小样本单次结果）。

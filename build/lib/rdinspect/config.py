...
def default_config_candidates() -> list[Path]:
    """默认配置的查找顺序：仓库内 → 当前工作目录 → 包内（pip 安装时只有这一条可能命中）。"""
...
def find_default_config() -> Path | None:
    return next((path for path in default_config_candidates() if path.exists()), None)


def sidecar_config(config_path: Path | None, name: str) -> Path | None:
    """prelabel.yaml / train.yaml 等旁挂配置：优先与主配置同目录，其次 CWD/configs。"""
    candidates = []
...
def _load_train(config_path: Path | None, raw: dict[str, Any]) -> TrainConfig:
    ...
    if isinstance(override, dict):
        ...
        candidate = sidecar_config(config_path, "train.yaml")
        train_raw = yaml.safe_load(candidate.read_text(encoding="utf-8")) if candidate else {}
...
def load_config(path: str | Path | None = None, *, data_dir: str | Path | None = None,
    ...
    if path:
        ...
        config_path = Path(os.environ["ROAD_INSPECT_CONFIG"])
    else:
        config_path = find_default_config()
        if config_path is None:
            # pip 安装（非 editable）时仓库里的 configs/ 并不存在：用内置默认值继续，
            # 而不是让 `rdinspect infer` 这类边缘命令直接报"配置文件不存在"。
            print("提示：未找到 configs/default.yaml，使用内置默认配置（可用 --config 指定）",
                  file=sys.stderr)
    if config_path is not None and not config_path.exists():
        raise ConfigError(f"配置文件不存在: {config_path}")
    raw = yaml.safe_load(config_path.read_text(encoding="utf-8")) or {} if config_path is not None else {}
    ...
    if not isinstance(prelabel_raw, dict):
        prelabel_path = sidecar_config(config_path, "prelabel.yaml")
        prelabel_raw = yaml.safe_load(prelabel_path.read_text(encoding="utf-8")) if prelabel_path else {}
